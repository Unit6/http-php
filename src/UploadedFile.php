...
class UploadedFile implements UploadedFileInterface
{
    ...
    /**
     * Create a normalized tree of UploadedFile instances from global
     * server variables. Parse a non-normalized, i.e. $_FILES superglobal,
     * tree of uploaded file data.
     *
     * @param array $uploadedFiles The non-normalized tree of uploaded file data.
     *
     * @return array A normalized tree of UploadedFile instances.
     */
    public static function parse(array $uploadedFiles = [])
    {
        if (empty($uploadedFiles) && isset($_FILES)) {
            ...
            foreach ($uploadedFiles as $field => $uploadedFile) {
                if (!isset($uploadedFile['error'])) {
                    if (is_array($uploadedFile)) {
                        $parsed[$field] = static::parseFrom($uploadedFile);
                    }
                    continue;
                }
                $parsed[$field] = [];
    ...
    public function moveTo($targetPath)
    {
        ...
        if ($targetIsStream) {
            ...
            if ( ! is_uploaded_file($this->file)) {
                throw new RuntimeException(sprintf('Error uploading file %1s', $this->file));
            }
     ...
     *
     * @see http://php.net/file-upload.errors
     *
